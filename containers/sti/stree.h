...
namespace sti
{
    ...
    namespace implementation
    {
        ...
        struct KeyWithGistImpl<Key, GIST_TRAITS, std::tr1::true_type>
        {
        ...
        struct KeyWrapperImpl<Key, GIST_TRAITS, Allocator, std::tr1::true_type> : protected Allocator
        {
            static const bool has_gist = false;

            typedef KeyWrapperImpl<Key, GIST_TRAITS, Allocator, std::tr1::true_type> this_type;
        ...
        template<class Key, class GIST_TRAITS, class Allocator>
        const Key& extract_key(const KeyWrapperImpl<Key, GIST_TRAITS, Allocator, std::tr1::false_type>& k) { return *k.v; }

        template<class Key, class GIST_TRAITS, class Allocator>
        const Key& extract_key(const KeyWrapperImpl<Key, GIST_TRAITS, Allocator, std::tr1::true_type>& k) { return k.v; }
        ...
        bool compare_key_helper(const KeyWrap1& l, const KeyWrap2& r, LESS comp, std::tr1::false_type)
        {
            return comp(extract_key(l), extract_key(r));
        }

        template<class KeyWrap1, class KeyWrap2, class LESS> 
        bool compare_key_helper(const KeyWrap1& l, const KeyWrap2& r, LESS comp, std::tr1::true_type)
        {
            if (l.gist==r.gist)
                return comp(extract_key(l), extract_key(r));
            else 
        ...
        struct LeafNodeImpl<Allocator, Key, value_type, KeyWrapper, key_extractor, std::tr1::true_type>
        {
        ...
        private:
            ...
            struct TBNode : public BaseBNode
            {
                ...
                TBNode()
                    : BaseBNode(std::tr1::is_same<N, LeafNode>::value)
                {}
...
#define FIND_UNLOOP_N 8
        ...
        public: // XXX
            ...
            private:
                ...
                {
                    typedef typename boost::enable_if<std::tr1::is_convertible<Other, V> >::type type;
                };
            public:
                iter() 
                    : _bnode(0)
...
#    define VALIDATE(x) do {} while(0)
        ...
        public:
            ...
            iterator insert(iterator _Where, const value_type& _Val)
            {
                ...
                std::tr1::tie(_Where, std::tr1::ignore) = insert(_Val);
                return _Where;
            }
