...
//https://raw.githubusercontent.com/sauloal/cnidaria/008096382312f5855336b1dde56e1a97df055ff1/src/headers/progressbar.hpp
...
class progressBar {
  ...
  public:
    ...
    progressBar( std::string namel ):
        name(namel)      , minValue(0), maxValue(0), lastPercent(9999), lastValue(0)   , lastPos(0),
    ...
    inline void sec_to_str(char * str, baseInt  secs){
      ...
      if ( v ) {
        ...
        if ( progress ) {
          ...
          if ( two_lines ) {
            ...
            //std::fprintf( stdout, "%s kmers: done %'13llu total %'13llu left %'13llu | time: ela %s etc %s | speed %'8llu kmer/s\n\n",
            std::fprintf( stdout, "%s kmers: done %'13llu total %'13llu left %'13llu | time: ela %s etc %s | speed %'10.0f kmer/s | %5.1lf%%\n",
